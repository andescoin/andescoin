...
#if defined(NDEBUG)
# error "AndesCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa5b215e52e9c088c8c834288d7ff4735d59916ef6e54ef408b64908767c05825");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AndesCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AndesCoin Signed Message:\n";
...
{
    // AndesCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AndesCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AndesCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1524583829, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "el nacional 03 abr 18 Almagro prevee abordar mas sanciones";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b17b590796d4aa1292add9729a94fa2c9dc786d9e882d3c1abce3fa7b1a910262e04aaa7122edd41605a8c5d5701224c0e7b3ddae423a3f1cbd9e87407dddf8f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524583829;
        ...
        assert(block.hashMerkleRoot == uint256("0xf1f074c54195fd330840191389a208c6e818efbe61853434d3efdc10e0b35682"));
...
unsigned char pchMessageStart[4] = { 0x7c, 0x2a, 0xe0, 0x89 }; // AndesCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AndesCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AndesCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AndesCoinMiner(CWallet *pwallet)
{
    printf("AndesCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("andescoin-miner");
    ...
    try { loop {
        ...
        printf("Running AndesCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AndesCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AndesCoinMiner, pwallet));
}
